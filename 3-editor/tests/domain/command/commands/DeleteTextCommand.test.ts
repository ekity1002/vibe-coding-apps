...
import { DeleteTextCommand } from '../../../../src/domain/command/commands/DeleteTextCommand'
...
describe('DeleteTextCommand', () => {
  ...
  describe('Execute Method', () => {
    it('should execute text deletion correctly', () => {
      ...
      expect(updateTextSpy).toHaveBeenCalledWith('Hellorld!')
    ...
    it('should handle special characters deletion', () => {
      ...
      // 実際の結果に合わせて期待値を更新
      const expectedResult = mockContext.currentText.slice(0, 6) + mockContext.currentText.slice(9)
      expect(updateTextSpy).toHaveBeenCalledWith(expectedResult)
      expect(command.getDeletedText()).toBe(mockContext.currentText.slice(6, 9))
    })
